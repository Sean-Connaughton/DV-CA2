...
// main
let colourConfigs = initialiseColourConfigs();
let buttonIcon = initialiseButtonIconConfig();

changeElementColours(colourConfigs);
changeButtonIcon(buttonIcon);

document.getElementById("changeColourModeButton").addEventListener("click", () => { swapConfigs(); changeElementColours(colourConfigs); changeButtonIcon(buttonIcon);});

// colour functions
function initialiseColourConfigs() {
...
function swapConfigs() {
    if (getCookie("colourScheme") == "Light") {
        // swap to dark
        colourConfigs.toRemove = lightConfigurations;
        colourConfigs.toAdd = darkConfigurations;
        buttonIcon = darkButtonIcon;
        document.cookie = colourSchemes.dark;
    }
    else if (getCookie("colourScheme") == "Dark" || getCookie("colourScheme" == "")) {
        // swap to light
        colourConfigs.toRemove = darkConfigurations;
        colourConfigs.toAdd = lightConfigurations;
...
function changeElementColours(colourConfigs) {
    for (let i = 0; i < pageElements.length; i++) {
        document.getElementById(pageElements[i]).classList.remove(colourConfigs.toRemove[i]);
        document.getElementById(pageElements[i]).classList.add(colourConfigs.toAdd[i]);
    }
}
